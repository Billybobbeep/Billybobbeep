...
module.exports = (message, db) => {
	...
	if (args[2].toLowerCase() === 'reset') {
		db.delete(message.guild.id + '.modRole');
		return message.channel.send('Removed moderator role from the database.');
	}
	let role = message.mentions.roles.first() || message.guild.roles.cache.get(args[2]);
	if (!role) {
		return message.channel.send(`Could not find the role \`${args[2]}\``)
	}
	if (role.id === db.get(message.guild.id + '.modRole')) return message.channel.send(`Your moderator role is already set as ${role}`)
	db.set(message.guild.id + '.modRole', role.id);
