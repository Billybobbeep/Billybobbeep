...
module.exports = (message, db) => {
  ...
  if (args[2] && args[2].toLowerCase() === 'reset') {
    if (db.get(message.guild.id + '.welcomeChannel')) {
      db.delete(message.guild.id + '.welcomeChannel')
  ...
  if (args[2] && args[2].toLowerCase() !== 'reset' && args[2].toLowerCase() !== 'help') {
    ...
    if (channel.id === db.get(message.guild.id + '.welcomeChannel')) return message.channel.send(`Your welcome channel is already set as ${channel}`)
    db.set(message.guild.id + '.welcomeChannel', channel.id)
