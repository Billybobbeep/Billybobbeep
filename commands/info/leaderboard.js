module.exports = {
  ...
  execute: function (message, prefix, client) {
    ...
    guildData.findOne({ guildId: message.guild.id }).then((guildResult) => {
      ...
      embed.setColor(
        guildResult.preferences ? guildResult.preferences.embedColor : "#447ba1"
      );
      ...
      guildMemberData.find(function (err, result) {
        if (err)
          return message.channel.send(
            "I do not have enough data to use this command"
          );
        if (args[1] && args[1] === "global") {
          ...
          finalData.forEach((data) => {
            i++;
            let user = client.users.cache.get(data.split("_")[1])
              ? client.users.cache.get(data.split("_")[1]).tag
              : false;

            if (!user) {
              guildMemberData
                .findOne({
                  guildId: data.split("_")[2],
                  memberId: data.split("_")[1],
                })
                .then((memberResult) => memberResult.delete());
              user = "Unknown";
            }
            ...
            embed.addField(
              `#${i}`,
              `${user} - **${data.split("_")[0]}** - ${
                client.guilds.cache.get(data.split("_")[2]).name
              }`,
              false
            );
          });
          ...
          embed.setFooter(
            'Use "' +
              prefix +
              'leaderboard global" to see the global leaderboard'
          );
          result.forEach((data) => {
