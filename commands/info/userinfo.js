  ...
  execute: function (message, prefix, _client) {
    ...
    function game() {
      ...
      if (user.presence.activities.length >= 1)
        ...
        game = "None"; // If the user is not playing a game.
      return game;
    }
    ...
    let createdate = moment
      .utc(user.createdAt)
      .format("dddd, MMMM Do YYYY, HH:MM:ss"); // User Created Date.
    let joindate = moment
      .utc(member.joinedAt)
      .format("dddd, MMMM Do YYYY, HH:mm:ss"); // User joined the server at.
    ...
    guildData.findOne({ guildId: message.guild.id }).then((result) => {
      const embed = new Discord.MessageEmbed()
        ...
        .addField(
          ...
          `${createdate} \nCreated ${created} day(s) ago`,
          true
        )
        .addField(
          "Join Server Date",
          `${joindate} \nJoined ${joined} day(s) ago`,
          true
        )
