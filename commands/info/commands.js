...
module.exports = {
    ...
    execute(message, prefix, client) {
        ...
        guildData.findOne({ guildId: message.guild ? message.guild.id : message.guild_id }).then(async result => {
                ...
                for (const folder of commandFolders) {
                    ...
                    for (const file of commandFiles) {
                        ...
                        catagories.forEach(catagory => {
                            if (command.usage && command.catagory && command.catagory == catagory) {
                                page.addField(
                                    `${prefix}${command.name}`,
                                    `${command.description}\n` +
            ...
            requesters.push(message.author ? message.author.id : message.member.user.id);
        });
    },
    /**
            ...
            componentsToChange.forEach(c => {
                (message.components).forEach(row => {
                    row.components.forEach(component => {
                        if (component.custom_id == c.name)
                            component.disabled = c.disabled;
                    });
                    components.push(row);
                });
            });
