    ...
    async execute (message, prefix, client) {
        ...
        let channel = message.data ? {
            ...
            async sendTyping() {
                ...
                if (!guild)
                    await client.guild.fetch(message.guild_id);
                let channel = guild.channels.cache.get(message.channel_id);
                if (!channel) return;
                channel.sendTyping();
            }
        } : message.channel;
        let args = !message.data ? message.content.slice(prefix.length).trim().split(/ +/g) : message.data.options;
        let user;
        ...
        if (!message.data && message.guild)
            user = message.mentions.users.first() || message.guild.members.cache.get(args[1]) || message.author;
