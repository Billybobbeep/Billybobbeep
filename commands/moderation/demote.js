  ...
  execute: function (message, prefix, client) {
    ...
    function demoteCmd() {
      guildData.findOne({ guildId: message.guild.id }).then(async (result) => {
        ...
        if (!user)
          return message.channel.send("You must mention a user to demote");
        if (!user.tag)
          user = user.user;
        ...
        if (member.roles.cache.get(result.preferences.modRole)) {
          await member.roles.remove(result.preferences.modRole).catch(() => {
            return message.channel.send("I do not have permissions to use this command");
          });
