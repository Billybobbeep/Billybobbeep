  ...
  execute: async function(interaction, client) {
    ...
    if (subcommand.name === "add-reaction") {
      ...
      if (guild.pendingReactionRoles.find(role => role.roleId === role))
        return interaction.followUp({ content: `This role is already pending to be inserted into an embed, use the \`/${this.name} create\` command to send the embed to a channel`, ephemeral: true });
      ...
      if (guild.pendingReactionRoles.length < 1)
        return interaction.followUp({ content: `There are no pending roles, ensure you have used the \`/${this.name} add-reaction\` before finalization`, ephemeral: true });
