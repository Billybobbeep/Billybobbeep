  ...
  execute: function (message, prefix, client) {
    ...
    function lockdown() {
      if (args[1]) {
        ...
        else if (args[1].includes("server") || args[1].includes("all"))
          channel = "all";
        else if (
          args[1].toLowerCase().includes("ms") || args[1].toLowerCase().includes("s") ||
          args[1].toLowerCase().includes("m") || args[1].toLowerCase().includes("h") || args[1].toLowerCase().includes("d")
        )
          duration = args[1];
      }

      if (!duration && !channel && args[1])
        args[1].includes("ms") || args[1].includes("s") || args[1].includes("m") || args[1].includes("h") || args[1].includes("d")
          ? (duration = ms(args[1]))
          : false;
      if (!duration && args[2])
        args[2].includes("ms") || args[2].includes("s") || args[2].includes("m") || args[2].includes("h") || args[2].includes("d")
          ? (duration = ms(args[2]))
          : false;
      ...
      guildData.findOne({ guildId: message.guild.id }).then((result) => {
        ...
        duration
          ? embed.setDescription(
            "This channel has been locked for " +
            duration +
            ".\nReason: " +
            reason
          )
          : embed.setDescription("This channel has been locked till further notice");
        ...
        if (channel !== "all") {
          if (channel.type === "text") {
            if (
              ...
              channel.permissionOverwrites.set(
                ...
                reason
                  ? reason + ` - ${message.author.tag} via ${prefix}${this.name}`
                  : `No reason was provided - ${message.author.tag} via ${prefix}${this.name}`
              );
            ...
            else message.channel.send(`${channel} is already locked`);
          } else
            channel.permissionOverwrites.set(
              [{
                id: message.guild.roles.cache.find((r) => r.name === "@everyone").id,
          ...
          message.guild.channels.cache.array().forEach((channel) => {
            ...
            if (channel.type === "text") {
              ...
              duration
                ? embed.setDescription(
                    "This channel has been locked for " + duration +
                    ".\nReason: " +
                    reason
                  )
                : embed.setDescription(
                    "This channel has been locked till further notice"
                  );
              channel.send({ embeds: [embed] });
            } else if (channel.type === "voice")
              channel.permissionOverwrites.set(
