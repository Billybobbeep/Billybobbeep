  ...
  execute: function (message, prefix, client) {
    ...
    function muteCmd() {
      guildData.findOne({ guildId: message.guild.id }).then(async (result) => {
        ...
        let user = message.guild_id
          ? await client.users.fetch(args[0].value)
        ...
        if (
          member.roles.cache.find((r) => r.id === result.preferences.mutedRole)
        )
          return message.channel.send(`<@!${user.id}> is already muted`);
        if (user.bot) return message.channel.send("You cannot mute bots");
        if (!result.preferences.mutedRole)
          return message.channel.send(
            "You must setup a muted role in your server to use this command"
          );
        if (!user)
          return message.channel.send("You must provide a user to mute");
        if (!user.tag) user = user.user;
        if (user.id === message.author.id)
          return message.channel.send("You cannot mute yourself");
        if (!member)
          return message.channel.send("I could not find the member you provided");
        if (!time)
          return message.channel.send("You must provide a time or reason");
        if (member.permissions.has(Discord.Permissions.FLAGS.ADMINISTRATOR))
          return message.channel.send("You cannot mute a server administrator");
        if (
          result.preferences?.modRole &&
          member.roles.cache.find((r) => r.id == result.preferences?.modRole)
        )
          return message.channel.send("You cannot mute a server moderator");
        try {
        ...
        if (!guild.roles.fetch((r) => r.id === result.preferences.mutedRole))
          return message.channel.send(
            "You must setup a muted role in your server to use this command"
          );
        member.roles
          .add(
            guild.roles.cache.find(
              (role) => role.id === result.preferences.mutedRole
            )
          )
          .then(() =>
            message.channel.send("Successfully muted <@!" + user.id + ">")
          )
          .catch((error) => {
            if (error.toString().includes("permissions"))
              return message.channel.send(
                "I cannot mute <@!" +
                  user.id +
                  ">. Make sure my highest role is above <@!" +
                  user.id +
                  ">'s highest role."
              );
            else {
        ...
        const newMutedMember = new mutedMembers({
          guildId: guild.id,
          userId: user.id,
          time: Date.now() + time,
        });
    ...
    guildData
      .findOne({ guildId: message.guild_id || message.guild.id })
      .then(async (result) => {
        let guild = await client.guilds.fetch(
          message.guild_id || message.guild.id
        );
        let member = guild.members.cache.get(
          !message.author ? message.member.user.id : message.author.id
        );
        if (
          member.permissions.has(Discord.Permissions.FLAGS.MANAGE_MESSAGES) ||
          ...
          if (
            message.member.roles.cache.find(
              (role) => role.id === result.preferences.modRole
            )
          ) {
            muteCmd();
            debounce = true;
