  ...
  execute: function (message, prefix, client) {
    ...
    function promoteCmd() {
      guildData.findOne({ guildId: message.guild.id }).then(async (result) => {
        ...
        if (!user)
          return message.channel.send("You must provide a user to promote");
        if (!user.tag) user = user.user;
        if (!result.preferences.modRole)
          return message.channel.send(
            "You need to set up a mod role in your server to use this command"
          );
        ...
        if (!member.roles.cache.get(result.preferences.modRole)) {
          await member.roles.add(result.preferences.modRole).catch(() => {
            return message.channel.send(
              "I do not have permissions to use this command"
            );
          });
          message.channel.send(
            `<@!${user.id}> was promoted by <@!${message.author.id}>`
          );
          ...
          try {
            ...
            message.channel.send(
              `The user was not notified as they do not have their DMs turned on`
            );
          }
          logging(embed, message, client);
        } else message.channel.send(`<@!${user.id}> is already a moderator`);
      });
    }
