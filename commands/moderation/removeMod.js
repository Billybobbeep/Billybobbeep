...
module.exports = {
  name: "removemod",
  description: "Disallow a user to have moderator permissions",
  category: "mod",
  slashInfo: {
    enabled: true,
    public: true
  ...
  getSlashInfo: function() {
    ...
    builder.addUserOption((option) => {
      ...
      option.setDescription("The user to remove as a moderator");
    ...
    builder.addStringOption((option) => {
      ...
      // Set the option description
      option.setDescription("The reason for removing the user as a moderator");
      // If the option is required
  ...
  execute: async function(interaction, client) {
    ...
    if (!guild.preferences.customMods) {
      ...
      let customMods = [...guild.preferences.customMods];
      if (!customMods.includes(user.id))
        return interaction.followUp({ content: `${user.username} has not been set as a custom moderator`, ephemeral: true });
      customMods.splice(customMods.indexOf(user.id), 1);
      guild.preferences.customMods = customMods;
    }
    ...
    embed.setTimestamp();
    // Set embed data
    embed.setTitle("Moderator Removed");
    ...
    interaction.followUp({
      content: [
        `Removed ${user.username}'s moderator permissions`,
        `**NOTE:** If this user has a moderator role, they will still have access to the moderator privileges`
      ].join("\n"),
      ephemeral: true });
  }
}