...
function help_embed(message, prefix) {
  guildData.findOne({ guildId: message.guild.id }).then((result) => {
    ...
    embed.setDescription(
      "**Flags:**\n\n" +
        "--filter-user-[user-id] - " +
        "Filters all logs by the user ID.\n" +
        "--filter-logs-[audit/bot] - " +
        "Filters logs by the bot or from the audit logs.*\n" +
        "--remove-action-[action] - " +
        "Remove certain actions from the table. (See below for available actions)\n" +
        "--filter-action-[action] - " +
        "Filter logs by a certain action. (See below for available actions)\n\n" +
        "* = required\n\n" +
        "**Actions**\n" +
        "`The following actions are only available with the audit log flag\n\n" +
        "emoji_update - " +
        "Add, remove or rename emojis.\n" + //EMOJI_DELETE, EMOJI_UPDATE, EMOJI_CREATE
        "role_update - " +
        "Add, remove or rename roles.\n" + //ROLE_UPDATE
        "member_role_update - " +
        "Member add or remove role.\n" + //MEMBER_ROLE_UPDATE
        "channel_update - " +
        "Channel added or removed.\n" + //CHANNEL_CREATE, CHANNEL_DELETE
        "bulk_delete - " +
        "Purged Messages.\n" + //MESSAGE_BULK_DELETE
        "ban_add - " +
        "Member banned.\n" + //MEMBER_BAN_ADD
        ...
        prefix +
        "logs --filter-logs[audit] --filter-action-[action1]-[action2]-[action3]`"
    );
...
function filter_flags(client, message, prefix) {
  ...
  if (!message.content.toLowerCase().includes("--filter-logs"))
    return message.channel.send("Logs flag was not found, make sure you have included all of the required flags");
  if (message.content.toLowerCase().includes("--filter-logs-bot")) {
    ...
    let actions =
      args.find(
        (f) =>
          f.startsWith("--filter-action-") || f.startsWith("--filter-actions-")
      ) || "Not filtered";
    if (actions !== "Not filtered") {
      actions = actions.substring(16);
      actions = actions.replace("-", " ").replace("-", " ").split(/ +/g);
      if (actions[2] && actions[2].includes("-"))
        return message.channel.send("You can only select up to `3` actions to filter");
      ...
      if (actions[0]) {
        ...
        else if (actions[0].toLowerCase() === "role_update") FA = "ROLE_UPDATE";
        ...
        else if (actions[0].toLowerCase() === "ban_add") FA = "MEMBER_BAN_ADD";
        else if (actions[0].toLowerCase() === "ban_remove")
          FA = "MEMBER_BAN_REMOVE";
        else if (actions[0].toLowerCase() === "kick_add") FA = "MEMBER_KICK";
        else if (actions[0].toLowerCase() === "bot_add") FA = "BOT_ADD";
        else
          return message.channel.send(
            `**${
              actions[0][0].toUpperCase() +
              actions[0].substring(1).toLowerCase()
            }** is not a valid action`
          );
      } else return message.channel.send("You have entered an invalid action");

      if (actions[1]) {
        if (actions[1].toLowerCase() === "emoji_update")
          SA = ["EMOJI_DELETE", "EMOJI_UPDATE", " EMOJI_CREATE"];
        else if (actions[1].toLowerCase() === "role_update") SA = "ROLE_UPDATE";
        ...
        else if (actions[1].toLowerCase() === "ban_add") SA = "MEMBER_BAN_ADD";
        else if (actions[1].toLowerCase() === "ban_remove")
          SA = "MEMBER_BAN_REMOVE";
        else if (actions[1].toLowerCase() === "kick_add") SA = "MEMBER_KICK";
        else if (actions[1].toLowerCase() === "bot_add") SA = "BOT_ADD";
        else
          return message.channel.send(
            `**${
              actions[1][0].toUpperCase() +
              actions[1].substring(1).toLowerCase()
            }** is not a valid action`
          );
      }
      ...
      if (actions[2]) {
        ...
        else if (actions[2].toLowerCase() === "role_update") TA = "ROLE_UPDATE";
        ...
        else if (actions[2].toLowerCase() === "ban_add") TA = "MEMBER_BAN_ADD";
        else if (actions[2].toLowerCase() === "ban_remove")
          TA = "MEMBER_BAN_REMOVE";
        else if (actions[2].toLowerCase() === "kick_add") TA = "MEMBER_KICK";
        else if (actions[2].toLowerCase() === "bot_add") TA = "BOT_ADD";
        else
          return message.channel.send(
            `**${
              actions[2][0].toUpperCase() +
              actions[2].substring(1).toLowerCase()
            }** is not a valid action`
          );
      }
      ...
      if (FA !== "" && SA !== "")
        if (FA === SA)
          return message.channel.send(
            `You have filtered the action **${actions[0]}** more than once.\nYou can only filter an action once`
          );
      if (SA !== "" && TA !== "")
        if (SA === TA)
          return message.channel.send(
            `You have filtered the action **${actions[1]}** more than once.\nYou can only filter an action once`
          );
      if (TA !== "" && FA !== "")
        if (TA === FA)
          return message.channel.send(
            `You have filtered the action **${actions[2]}** more than once.\nYou can only filter an action once`
          );
    ...
    actions =
      args.find(
        (f) =>
          f.startsWith("--remove-action-") || f.startsWith("--remove-actions-")
      ) || "Not filtered";
    if (actions !== "Not filtered") {
      actions = actions.substring(16);
      actions = actions.replace("-", " ").replace("-", " ").split(/ +/g);
      if (actions[2] && actions[2].includes("-"))
        return message.channel.send(
          `You can only select up to \`3\` actions to remove`
        );
...
function audit_logs(client, message, action1, action2, action3) {
  message.guild.fetchAuditLogs().then((logs) => {
    ...
    if (action1 !== undefined && action2 === undefined && action3 === undefined)
      log = log
        .filter((e) => e.executor.id !== client.user.id)
        .filter((e) => e.action === action1);
    else if (
      action1 !== undefined &&
      action2 !== undefined &&
      action3 === undefined
      ...
      log = log
        .filter((e) => e.executor.id !== client.user.id)
        .filter((e) => e.action === action1 || e.action === action2);
    else if (
      action1 !== undefined &&
      action2 !== undefined &&
      action3 !== undefined
      ...
      log = log
        .filter((e) => e.executor.id !== client.user.id)
        .filter((e) => e.action === action1 || e.action === action3);
...
module.exports = {
  ...
  execute(message, _prefix, client) {
    if (
      message.member.roles.cache.has(["VIEW_AUDIT_LOG", "ADMINISTRATOR"]) &&
      !message.member.roles.cache.find(
        (role) =>
          role.id ===
          guildData
            .findOne({ guildId: message.guild.id })
            .then((result) => result.preferences.modRole)
      )
    )
      return message.channel.send(
        "You must have the `View Audit Log` permissions to use this command"
      );
