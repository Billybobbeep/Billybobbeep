    ...
    guildData.findOne({ guildId: message.guild.id }).then((result) => {
      if (!result)
        return message.channel.send({
          content: "This server has not been set up to use this command",
        });
      let data = {
      ...
      if (!data.total || !data.member || !data.bots)
        return message.channel.send({
          content: "This server has not been set up to use this command",
        });
      ...
      try {
        client.channels.cache
          .get(data.total)
          .setName(`${tu} ${message.guild.memberCount}`);
        client.channels.cache
          .get(data.member)
          .setName(`${tm} ${message.guild.members.cache.filter((m) => !m.user.bot).size}`
          );
        client.channels.cache
          .get(data.bots)
          .setName(`${tb} ${message.guild.members.cache.filter((m) => m.user.bot).size}`);
        message.channel.send({
          content: "Successfully updated the server stats",
        });
      } catch {
        message.channel.send({
          content: "There was an error whilst updating the server stats",
        });
      }
    });
  },
