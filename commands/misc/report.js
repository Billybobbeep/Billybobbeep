  ...
  execute: function (message, _prefix, client) {
    ...
    guildData.findOne({ guildId: message.guild.id }).then(async (result) => {
      ...
      embed4.setDescription(
        `Any additional information the moderator should know?`
      );
      ...
      if (message.channel.type === "dm") {
        ...
        if (msg && msg.id) {
          ...
          msg
            .awaitReactions(rfilter, { max: 1, time: 30000, errors: ["time"] })
            .then((collected) => {
              message.channel.send(embed2);
            });
          const mfilter = (collected) =>
            collected.author.id === message.author.id;
          let collected = await msg.channel
            .awaitMessages(mfilter, {
              max: 1,
              time: 50000,
            })
            .catch(() => {
              message.author.send(
                "Time expired, to continue, run this command again"
              );
            });
          ...
          if (
            ...
            return message.author.send("Canceled prompt");
          if (
            collected.first().content.toLowerCase() === "skip" ||
            collected.first().content.toLowerCase() === "~skip"
          )
            message.author.send("This is a required question");
          ...
          embed5.setDescription(
            "React to the message below if you are happy with the information below"
          );
          ...
          let tick = client.emojis.cache.get(
            require("../../utils/config.json").TickEmoji
          );
          let cross = client.emojis.cache.get(
            require("../../utils/config.json").CrossEmoji
          );
          ...
          const rfilter2 = (reaction, user) => {
            return (
              [tick.id, cross.id].includes(reaction.emoji.id) &&
              user.id !== client.user.id
            );
          };

          msg
            .awaitReactions(rfilter2, { max: 1, time: 30000, errors: ["time"] })
            .then((collected) => {
        ...
        let msg = await message.author.send({ embeds: [embed] }).catch(() => {
          return message.channel.send(
            "You must have your DMs turned on to use this command"
          );
        });
