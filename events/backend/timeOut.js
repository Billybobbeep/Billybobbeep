...
function mute(client) {
  ...
  MM.find(function (err, data) {
    ...
    data.forEach((result) => {
      if (
        cache.lastUnmuted &&
        cache.lastUnmuted.u == result.userId &&
        cache.lastUnmuted.g == result.guildId
      )
        return;
...
async function remove(result, guild, user, string, client) {
  ...
  if (string == "mute") {
    ...
    embed.setTimestamp();
    embed.setColor(
      guildRes.preferences ? guildRes.preferences.embedColor : "#447ba1"
    );
    embed.setDescription(
