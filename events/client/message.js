...
function redirect(message, client) {
  ...
  } else require("../backend/dmRecieving.js")(message, client); // Record incoming DMs
...
function handleMessage(message, client) {
  ...
  if (!message.guild && client.commands.get(command))
    return message.channel.send(
      `${client.user.username} does not support DM commands, consider using commands via a mutual server`
    );
  ...
  guildData.findOne({ guildId: message.guild.id }).then((result) => {
    ...
    if (message.guild) prefix = result?.prefix || "~";
    else prefix = "~";
    ...
    if (client.commands.get(command)) {
      ...
      if (
        message.guild &&
        client.commands.get(command).catagory &&
        client.commands.get(command).catagory == "moderation"
      ) {
        if (
          !require("../../utils/functions").guildPerms.clientpermissions(
            client,
            message,
            client.commands.get(command)
          )
        )
          require("../../utils/functions").guildPerms.permissionCallback(
            message,
            client,
            client.commands.get(command).permissions || "UNKNOWN"
          );
      }
      try {
        ...
        if (
          err
            .toString()
            .includes("client.commands.get(...).execute is not a function")
        )
          return;
  ...
  if (client.commands.get(command)) {
    ...
    if (client.commands.get(command).guildOnly && !interaction.guild_id)
      return require("../../utils/functions").slashCommands.reply(
        interaction,
        client,
        `The command \`/${command}\` is a guild-only command, consider using this command via a mutual server`
      );
    // If the command catagory is for moderators only, check the client has the correct permissions to actually execute the command
    if (
      typeof client.commands.get(command).catagory == "string" &&
      client.commands.get(command).catagory == "moderation"
    ) {
      if (
        !require("../../utils/functions").slashCommands.clientpermissions(
          client,
          interaction,
          client.commands.get(command)
        )
      )
        require("../../utils/functions").slashCommands.permissionCallback(
          interaction,
          client,
          client.commands.get(command).permissions || "UNKNOWN"
        );
...
function handleButtonClick(interaction, client) {
  ...
  if (
    ...
    typeof client.commands.get(interaction.data.custom_id.split("-")[0])
      .buttonCallback == "function"
  ) {
    client.commands
      .get(interaction.data.custom_id.split("-")[0])
      .buttonCallback(interaction, client);
  }
}
...
module.exports = function (message, client) {
  if (!message.interaction && message.author) {
    ...
    if (
      require("../../utils/cache").dev.get() &&
      message.guild_id !== require("../../utils/config.json").DevServer
    )
      return;
    handleButtonClick(message, client); // Handle interaction
  } else if (message.data) {
    // If the message type is a slash command
    // If the bot is in dev mode and the server is not the dev server, return
    if (
      require("../../utils/cache").dev.get() &&
      message.guild_id !== require("../../utils/config.json").DevServer
